#!/usr/bin/env python3
"""
GPU-Based Origami Attention Correlation Test
=============================================

Runs flash attention kernels on GPU with multiple tile configurations,
then compares measured performance against origami's analytical predictions.

Validates that origami's roofline model correctly ranks tile configurations
by measuring actual GPU latency with a parametric Triton flash attention kernel.

Output: results.json + correlation plots in output directory.

Usage:
  python3 bench_attention_correlation.py [--quick] [--output-dir DIR]
"""
...
def spearman_rho(x, y):
    """Pure-Python Spearman rank correlation with proper tie handling.

    Uses midrank averaging for tied values, matching scipy.stats.spearmanr.
    The previous implementation used the simplified formula
    1 - 6*d²/(n*(n²-1)) which is only correct when there are no ties.
    Analytical latency predictions frequently produce ties (e.g. symmetric
    tile configs like 32x64 and 64x32 can map to the same roofline estimate),
    so tie-aware ranking is essential for accurate correlation.
    """
    n = len(x)
    if n < 3:
        return 0.0
    ...
    def midrank(arr):
        """Assign midranks (average rank for tied values)."""
    ...
    # Pearson correlation on ranks (general Spearman formula)
    mean_rx = sum(rx) / n
    mean_ry = sum(ry) / n
...
def generate_plots(results, output_dir):
    ...
    fig.suptitle('Origami Attention Correlation — Predicted vs Measured Latency\n'
    ...
    fig.suptitle('Attention Correlation Summary — GPU Measurements vs Analytical Model',
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description="GPU Attention Correlation Test")
    parser.add_argument("--output-dir", default="/tmp/origami_attn_corr",
    ...
    print("=" * 70)
    print("GPU ATTENTION CORRELATION TEST")
    print("=" * 70)
    ...
    # ── Save CSV ─────────────────────────────────────────────────────────
    csv_path = os.path.join(args.output_dir, "measured_latency.csv")
