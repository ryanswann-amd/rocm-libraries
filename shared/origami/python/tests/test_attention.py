...
@pytest.mark.integration
def test_att_compute_cvt_overhead(hardware):
    """Test conversion overhead calculation for attention."""
    problem = create_attention_problem(2048, 2048, 128, dtype="f32")
    config = create_attention_config(128, 128, 64)

    # Signature: compute_cvt_overhead(problem, hardware, config)
    overhead = origami.att_compute_cvt_overhead(problem, hardware, config)
    assert overhead >= 0.0
...
def test_att_check_lds_capacity(hardware):
    ...
    # Signature: check_lds_capacity(hardware, mt, a_dtype, b_dtype)
    mt_small = origami.dim3_t(64, 64, 32)
    assert origami.att_check_lds_capacity(hardware, mt_small, problem.a_dtype, problem.b_dtype) == True

    # Very large config that might not fit
    mt_large = origami.dim3_t(512, 512, 128)
    result = origami.att_check_lds_capacity(hardware, mt_large, problem.a_dtype, problem.b_dtype)
