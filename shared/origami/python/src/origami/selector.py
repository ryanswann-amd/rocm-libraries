...
Supports both TensileLite (hipBLASLt) and Triton backend targets.
"""
...
class OrigamiMatmulSelector:
    ...
    Supports two modes:
    - TensileLite (hipBLASLt) mode: External config_gen provides candidate configs.
      All existing behavior is preserved unchanged.
    - Triton mode: Generates configs internally with Triton-specific LDS filtering,
      tile heuristics, StreamK grid, and work-stealing parameters.

    The mode is controlled by the `target` parameter (default: "tensilelite").
    ...
    # Work-stealing default: overridden by _select_ws_params() for Triton
    COUNTERS_PER_XCD = 4

    def __init__(
        self,
        config_gen: Optional[Iterable] = None,
        m: int = 0,
        n: int = 0,
        k: int = 0,
        ...
        Args:
            config_gen: Iterable of Triton-style config objects with kwargs containing
                       ...
                       Required for tensilelite target, optional for triton target.
            ...
            target: Backend target, either "tensilelite" or "triton" (default: "tensilelite").
            total_cus: Override total CU count (for CU-mask testing, Triton only).
            active_cus: Active CU count under CU mask (Triton only).
        ...
        # --- Triton-only: detect architecture name for MI instruction selection ---
        if self._is_triton:
            ...
            # Save active CU info for Triton Stream-K
            self._active_cus = active_cus

            # When running under a CU mask, override with real total
        ...
        # --- Create Origami problem_t ---
        self._problem = self._make_problem()

        # --- Config generation: diverges by target ---
        if self._is_triton:
            # Triton path: generate configs internally, apply Triton LDS filter
            self._block_mn_range = [16, 32, 64, 128, 256]
            self._block_k_range = [16, 32, 64, 128, 256, 512]
            self._kernel_occupancy_range = [1]
            self._configs = self._generate_triton_configs()

            # Apply Triton LDS filtering (multi-stage pipeline model)
            ...
            if not self._configs:
                # Fallback: origami's raw check is more permissive
                self._configs = self._generate_triton_configs()
                self._configs = [
                    c
                    for c in self._configs
            ...
            # TensileLite path: use external config_gen (original behavior, unchanged)
        ...
        # --- Triton-only: 256x256x64 tile heuristic ---
        if self._is_triton:
            bytes_a = self._a_dtype_bitsize / 8
            bytes_b = self._b_dtype_bitsize / 8
            lds_cap = self._hardware.lds_capacity
        ...
        if self._is_triton:
            if streamk:
                self._grid = self._compute_triton_sk_grid()
            else:
        ...
        # --- Triton-only: work-stealing parameter override ---
        if self._is_triton:
            self._select_ws_params()


    # -------------------------------------------------------------------------
    # Properties — shared by both targets
    # -------------------------------------------------------------------------
    ...
    def even_k(self):
        ...
        if self._is_triton:
            return self._k % self.macrotile_k == 0
    ...
    # -------------------------------------------------------------------------
    # Triton-only properties
    # -------------------------------------------------------------------------

    @property
    def block_m(self):
        """Alias for macrotile_m (Triton naming convention)."""
        return self._result.config.mt.m

    @property
    def block_n(self):
        """Alias for macrotile_n (Triton naming convention)."""
        return self._result.config.mt.n

    @property
    def block_k(self):
        """Alias for macrotile_k (Triton naming convention)."""
        return self._result.config.mt.k

    @property
    def group_m(self):
        """Workgroup mapping (Triton naming convention)."""
        return self._workgroup_mapping.wgm

    @property
    def num_sms(self):
        """XCC workgroup mapping (Triton naming convention)."""
        return self._workgroup_mapping.wgmxcc

    @property
    def num_stages(self):
        """Pipeline stages for Triton kernel."""
        return self._num_stages

    @property
    def waves_per_eu(self):
        """Occupancy (Triton naming convention)."""
        return self._result.config.occupancy

    @property
    def sk_grid(self):
        """StreamK grid size (Triton naming convention)."""
        return self._grid

    # -------------------------------------------------------------------------
    # TensileLite-only: tile shortlists (unchanged from original)
    # -------------------------------------------------------------------------
    ...
    def _generate_configs(self, config_gen) -> list:
        """
        Convert Triton-style configs to Origami config_t objects with
        dimension-aware prefiltering.

        Used ONLY for TensileLite target. Triton uses _generate_triton_configs().
    ...
    def _generate_triton_configs(self) -> list:
        """
        Generate default Triton GEMM configs internally.

        Uses architecture-aware matrix instruction inference and generates
        all combinations of block sizes and occupancy values. Configs are
        tagged with target_t::triton so the C++ layer applies Triton-specific
        logic (LDS checks, short-circuit gating).
    ...
    def _infer_matrix_instruction_dimensions(self):
        """
        Infer the matrix instruction dimensions based on hardware and dtype.

        Triton-specific: uses architecture name detection to select the correct
        MFMA instruction dimensions for the data type combination.

        Returns:
            origami.dim3_t: Matrix instruction dimensions [M, N, K].

        Raises:
            ValueError: If the hardware/dtype combination is unsupported.
        """
        ...
        if mi_dim is None:
            raise ValueError(
                f"No Valid Matrix Instruction for {self._a_dtype_bitsize}-bit/"
                f"{self._b_dtype_bitsize}-bit dtypes "
    ...
    def _select_ws_params(self):
        ...
        self._workgroup_mapping.wgm = min(8, tiles_m)

    def hierarchical_split(self, num_xcds: int) -> tuple:
        """Compute optimal local/global tile split for hierarchical WS.

        Triton-only. Uses the full hardware CU count (not active CUs) so that
        the split is a topology-level constant, avoiding Triton recompilation
        when the active CU mask changes.
    ...
    def _compute_triton_sk_grid(self):
    ...
    def _partial_tile_size(self, sk_grid: int) -> int:
        """Compute partial tile workspace size for StreamK."""
    ...
    # -------------------------------------------------------------------------
    # Triton-specific: static LDS estimation methods
    # -------------------------------------------------------------------------
    ...
    def _make_problem(self) -> origami.problem_t:
        ...
        if self._is_triton:
            # Triton: hardcoded TN layout, no stride analysis, batch=1
            a_transpose = origami.transpose_t.T
            b_transpose = origami.transpose_t.N
        else:
            # TensileLite: calculate transpose types based on tensor strides
    ...
    # -------------------------------------------------------------------------
    # Transpose detection (TensileLite only)
    # -------------------------------------------------------------------------
    ...
    def _check_transpose_type(
        ...
        TensileLite only. Triton uses hardcoded TN layout.

        Args:
            shape: Tuple of (m, n) dimensions for the matrix.
            strides: Optional tuple of PyTorch tensor strides.
            default: Transpose type to use when strides is None.

        Returns:
            origami.transpose_t: The determined transpose type.

        Raises:
            ValueError: If the matrix has a non-dense memory layout.
        """
