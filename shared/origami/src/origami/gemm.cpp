...
size_t compute_number_matrix_instructions(const hardware_t& hardware, size_t MT_M, size_t MT_N,
                                          size_t MT_K, size_t MI_M, size_t MI_N, size_t MI_K,
                                          bool debug) {
...
size_t compute_mt_compute_latency(const hardware_t& hardware, size_t M, size_t N, size_t K,
                                  bool transA, bool transB, size_t MT_M, size_t MT_N, size_t MT_K,
                                  size_t MI_M, size_t MI_N, size_t MI_K, size_t element_size_A,
                                  size_t element_size_B, data_type_t mi_datatype, bool debug) {
    // Compute the number of matrix instructions
    size_t N_MI =
        compute_number_matrix_instructions(hardware, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K, debug);
    // Latency of a single MT_MxMT_NxMT_k tile is the latency of one MI multiplied by number of MI
    // per MT_MxMT_NxMT_k.
    ...
    // TN
    if (transA && !transB) {
        // We want to penalize tiles that can't be coalesced for T,N where K is contiguous
        // dimension. In this case, that's when the K dimension is indivisible by 128 bytes.
    ...
    if (!transA && transB) {
        // LDS Load Granularity is 128 Bytes -> If we load an amount indivisible by 128 bytes in
        // either contiguous dimesion from LDS then we will get poor LDS utilization. This actually
        // happens as more like a quantization effect where if either contiguous dimension of the
        // tile is not evenly divisible by 128-bytes We end up with inefficient loads.
        // Multiplication by a value is arbitrary, there is probably a better analytical method to
        // quantify the true impact of this Effect on the efficiency of computation.
...
size_t compute_number_waves(const hardware_t& hardware, size_t M, size_t N, size_t batch,
                            size_t MT_M, size_t MT_N, size_t split, bool debug) {
...
size_t compute_active_cu(const hardware_t& hardware, size_t M, size_t N, size_t batch, size_t MT_M,
                         size_t MT_N) {
    size_t num_mt_m = safe_ceil_div(M, MT_M);
    size_t num_mt_n = safe_ceil_div(N, MT_N);
    ...
    if (total_output_mt > hardware.N_CU) {
        ...
        return total_output_mt;  // If we don't have a full wave, return the number of MT (Data
                                 // parallel)
    }
}
...
double compute_bw_limite_from_occupancy(const hardware_t& /*hardware*/, size_t active_cu) {
...
double compute_memory_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, bool transA,
                              bool transB, size_t batch, size_t MT_M, size_t MT_N, size_t MT_K,
                              size_t split, double H_mem1, size_t element_size_A,
                              size_t element_size_B, size_t mx_block_size, bool debug) {
    double H_mem2 = estimate_mall_hit(hardware, M, N, K, batch, MT_M, MT_N, MT_K, /*WGM=*/1);
    ...
    if (active_cu < hardware.N_CU) {
        double min_load = static_cast<double>(M * MT_K * safe_ceil_div(element_size_A, 8) +
    ...
    if (debug || hardware_t::is_debug_enabled()) {
        ...
        hardware.log_debug("Macro_Tile", std::to_string(int(MT_M)) + "x" +
                                             std::to_string(int(MT_N)) + "x" +
...
double compute_tile_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                            bool transA, bool transB, size_t MT_M, size_t MT_N, size_t MT_K,
                            size_t MI_M, size_t MI_N, size_t MI_K, size_t split, double H_mem1,
                            size_t element_size_A, size_t element_size_B, size_t element_size_out,
                            data_type_t mi_datatype, size_t mx_block_size, bool debug) {
    // 1) Compute per-tile latencies
    double L_compute =
        compute_mt_compute_latency(hardware, M, N, K, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N,
                                   MI_K, element_size_A, element_size_B, mi_datatype, debug);

    double L_mem =
        compute_memory_latency(hardware, M, N, K, transA, transB, batch, MT_M, MT_N, MT_K, split,
                               H_mem1, element_size_A, element_size_B, mx_block_size, debug);
    ...
    double L_epilogue =
        (static_cast<double>(active_cu) * MT_M * MT_N * safe_ceil_div(element_size_out, 8)) /
        limited_mem1;

    // K-Split reductions are globally coherent, we need to write and read split-1 MT_M*MT_N tiles
    // to coherent memory
    if (split > 1) {
        size_t n_partials = split - 1;
        double partial_readwrite_bytes =
            (2 * active_cu * safe_ceil_div(element_size_out, 8) * MT_M * MT_N * n_partials);
    ...
    if (debug || hardware_t::is_debug_enabled()) {
        hardware.log_debug("Problem_Size", std::to_string(int(M)) + "x" + std::to_string(int(N)) +
                                               "x" + std::to_string(int(K)));
        hardware.log_debug("Macro_Tile", std::to_string(int(MT_M)) + "x" +
                                             std::to_string(int(MT_N)) + "x" +
                                             std::to_string(int(MT_K)));
...
double estimate_l2_hit(const hardware_t& hardware, int M, int N, int K, int batch, int MT_M,
                       int MT_N, int MT_K, int WGM, size_t element_size) {
...
double estimate_mall_hit(const hardware_t& hardware, int M, int N, int K, int batch, int MT_M,
                         int MT_N, int MT_K, int WGM) {
...
double compute_wave_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                            bool transA, bool transB, size_t MT_M, size_t MT_N, size_t MT_K,
                            size_t MI_M, size_t MI_N, size_t MI_K, size_t split, double H_mem1,
                            size_t element_size_A, size_t element_size_B, size_t element_size_out,
                            data_type_t mi_datatype, size_t mx_block_size, bool debug) {
    ...
    double L_wave = compute_tile_latency(
        hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K, split, H_mem1,
        element_size_A, element_size_B, element_size_out, mi_datatype, mx_block_size, debug);
...
double compute_total_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                             bool transA, bool transB, size_t MT_M, size_t MT_N, size_t MT_K,
                             size_t MI_M, size_t MI_N, size_t MI_K, size_t split, double H_mem1,
                             size_t element_size_A, size_t element_size_B, size_t element_size_out,
                             data_type_t mi_datatype, int WGM, size_t mx_block_size, bool debug) {
    ...
    {
        // If this is the case, we assume we're going to try and get an even split that fills the
        // most CUs.
    ...
    H_mem1 = estimate_l2_hit(hardware, M, N, K, batch, MT_M, MT_N, MT_K, WGM, element_size_A);
    ...
    double L_wave = compute_wave_latency(
        hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K, split, H_mem1,
        element_size_A, element_size_B, element_size_out, mi_datatype, mx_block_size, debug);

    // Short circuit conditions for tiles.
    if (enable_heuristics) {
        // Check if tile size is bigger than problem dimension. Invalidate it if it's not MI
        // dimension (smallest possible dimension)
...
double compute_perf_gflops(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                           bool transA, bool transB, size_t MT_M, size_t MT_N, size_t MT_K,
                           size_t MI_M, size_t MI_N, size_t MI_K, size_t element_size_A,
                           size_t element_size_B, size_t element_size_out, data_type_t mi_datatype,
                           int WGM, double H_mem1, bool debug) {
    // Compute total FLOPs
    ...
    double latency_cycles = compute_total_latency(
        hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K, 1, H_mem1,
        element_size_A, element_size_B, element_size_out, mi_datatype, WGM, mx_block_size, debug);
...
bool check_LDS_capacity(const hardware_t& hardware, size_t MT_M, size_t MT_N, size_t MT_K,
                        size_t element_size, bool debug) {
    // A and B size
    size_t Ld_A_value = compute_A_loads(MT_M, MT_K, debug);
    size_t Ld_B_value = compute_B_loads(MT_N, MT_K, debug);
