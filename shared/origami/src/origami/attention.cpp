...
double compute_total_latency(const problem_t& problem,
  ...
  // Total work = batch * q_heads * grid_M * grid_N * grid_K tiles distributed across N_CU CUs
  double total_tiles = static_cast<double>(batch) *
                       static_cast<double>(problem.q_heads) *
  ...
  if (debug) {
    ...
    OLOG_DEBUG("Batch: " << batch << " Q_Heads: " << problem.q_heads);
